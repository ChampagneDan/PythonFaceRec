...
matched = 0
access = 0
passwords = [('Dan', '1'), ('dad', 'dad'), ('mom', 'mom')]
...
def Verified():
    ...
    T.pack()
    T.insert(END, "Verified, You have access.")
    return Verified
...
def UnVerified():
    ...
    T = Text(root, height = 2, width = 30)
    T.pack()
    T.insert(END, "Error, Cannot verify face.")
    return UnVerified
...
def check_password(failures=[]):
    ...
    if (user.get(), password.get()) in passwords:
        ...
        ref_image = face_recognition.load_image_file("C:/Users/hamle_000/Desktop/" + ref_name + ".jpg")
